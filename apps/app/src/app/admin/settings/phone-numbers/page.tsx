...
import { PlusIcon, SaveIcon, XIcon, PhoneIcon } from "lucide-react";
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="w-full max-w-md rounded-lg bg-card p-6 shadow-lg">
            <div className="mb-4 flex items-center justify-between">
              ...
              <button onClick={() => setShowForm(false)} className="rounded p-1 hover:bg-muted"><XIcon className="size-5" /></button>
            </div>
            <form onSubmit={(e) => { e.preventDefault(); upsertMutation.mutate(); }} className="space-y-4">
              <div className="space-y-1">
                ...
                <input value={form.number} onChange={(e) => setForm({ ...form, number: e.target.value })} className="input-field" placeholder="+15555550100" required />
              </div>
              <div className="space-y-1">
                <label className="text-sm font-medium">Provider SID</label>
                <input value={form.providerSid} onChange={(e) => setForm({ ...form, providerSid: e.target.value })} className="input-field" placeholder="PNxxxxxxxx" />
              </div>
              <div className="space-y-1">
                <label className="text-sm font-medium">Forwarding Number</label>
                <input value={form.forwardingNumber} onChange={(e) => setForm({ ...form, forwardingNumber: e.target.value })} className="input-field" placeholder="+15555550999" />
              </div>
              <label className="flex items-center gap-2 text-sm">
                <input type="checkbox" checked={form.isActive} onChange={(e) => setForm({ ...form, isActive: e.target.checked })} className="rounded" />
                Active
              </label>
              {upsertMutation.isError && <p className="text-sm text-destructive">Failed to save</p>}
