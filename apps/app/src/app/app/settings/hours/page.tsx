    ...
    <div className="space-y-6">
      ...
      <div className="max-w-2xl space-y-3">
        {hours.map((hour, i) => (
          ...
          >
            <label htmlFor={`working-hours-${hour.dayOfWeek}`} className="flex w-28 items-center gap-2 text-sm font-medium">
              <Checkbox
                id={`working-hours-${hour.dayOfWeek}`}
                checked={hour.isActive}
                onCheckedChange={(checked) => updateHour(i, { isActive: checked })}
              />
              {DAYS[hour.dayOfWeek]}
            </label>
            <input
